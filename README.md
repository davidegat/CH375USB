...
**0.5.0 is published as a source release. No prebuilt 0.5.0 binaries are committed in the repository.** Build the driver from source with the supplied scripts.
...
- `CH375USB.ASM` — unified resident DOS/Windows-aware driver and block-device interface.
- `CH375MOU.ASM` — independently written Win16 mouse-driver bridge.
...
The repository does not contain the vendor `CH375DOS.SYS` or FreddyV's `CH375286.SYS` binary.

## Basic DOS installation

Build `CH375USB.SYS`, copy it to the DOS boot drive and add it to `CONFIG.SYS`:
